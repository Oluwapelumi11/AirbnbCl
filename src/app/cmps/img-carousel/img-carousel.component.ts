...
export class ImgCarouselComponent implements OnInit {
  ...
  isLikeActive() {
    ...
    else this.isLikeByUser = this.stay.likedByUsers.includes(user._id.toString())
  }
  ...
  async onClickLike(ev: Event) {
    ...
    try {
      ...
      if (!user) this.snackBar.open('Please login first', 'Close', { duration: 3000 })
        ...
        if (this.isLikeByUser) this.stay.likedByUsers = this.stay.likedByUsers.filter(userId => userId !== user._id.toString())
          else this.stay.likedByUsers.push(user._id.toString())
        console.log(user)
      console.log(this.stay)
        this.stay = await this.stayService.saveStay(this.stay) as Stay
        this.isLikeByUser = !this.isLikeByUser
        console.log(this.isLikeByUser)
